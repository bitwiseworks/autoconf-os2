...
ac_link='eval $ERLC $ERLCFLAGS -b beam conftest.$ac_ext >&AS_MESSAGE_LOG_FD && echo \"[#]!/bin/sh\" > conftest$ac_exeext && AS_ECHO([\"\\\"$ERL\\\" -run conftest start -run init stop -noshell\"]) >> conftest$ac_exeext && chmod +x conftest$ac_exeext'
])
